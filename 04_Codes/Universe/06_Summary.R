...
bluebook.delivery <- proj.price %>% 
  group_by(year, date, province, city, atc4, molecule, packid) %>% 
  ...
  select(Province = province, City = city, Channel, Molecule = molecule, 
         ...
         Year = year, Date = date, Volume, Value = sales)

write_feather(bluebook.delivery, '03_Outputs/Universe/06_Bluebook_2020_Universe_Summary.feather')
write.csv(bluebook.delivery, '03_Outputs/Universe/06_Bluebook_2020_Universe_Summary.csv', row.names = FALSE)
